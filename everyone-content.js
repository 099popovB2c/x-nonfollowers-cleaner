(() => {
  ...
  function findFollowingButton(cell) {
    const explicit = cell.querySelector('[data-testid$="-unfollow"], [data-testid="unfollow"]');
    if (explicit) return explicit;
    const labels = ["following","takip ediliyor","abonné","siguiendo","folge ich","segui","seguindo","volgend"];
  ...
  function findConfirmButton() {
    ...
    const words = ["unfollow","takibi bırak","takibi birak","ne plus suivre","dejar de seguir","entfolgen","smetti di seguire","deixar de seguir"];
  ...
  async function unfollowOne(cell, username) {
    ...
    if (!followingButton) return { ok: false, username, reason: "following-button-not-found" };
  ...
  async function run(limitRequested) {
    ...
    if (remainingToday <= 0) return { ok: false, reason: "daily-ceiling-reached", removed: 0, dailyCount: daily };

    const limit = Math.max(1, Math.min(remainingToday, Number(limitRequested) || remainingToday));
    const queue = [];
    for (const cell of getUserCells()) {
      const username = extractUsername(cell);
      if (!username || !findFollowingButton(cell)) continue;
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message?.type !== "XNFC_UNFOLLOW_EVERYONE") return;
    if (running) {
    ...
    run(message.limit)
      .then(sendResponse)
      .catch(error => sendResponse({ ok: false, reason: "unexpected-error", error: String(error) }))
